...
**Linux builds are made on the oldest distribution echo supports, never on a developer's own.**
...
Debian stable refuses to start it. `release.yml` builds on `ubuntu-22.04` — glibc 2.35, under every
distribution carrying the webkit2gtk 4.1 that Tauri v2 requires — and its `glibc floor` step reads
the built binary's symbols back and fails if they ask for more. Raising that number is a decision to
drop distributions, so it belongs in a diff.
