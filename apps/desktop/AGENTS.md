...
`.github/workflows/check.yml` runs the three commands above on every push, and
`.github/workflows/release.yml` builds every installer on a tag. Neither opens the window.

**Linux builds are made on the oldest distribution that can build them, never on a developer's own.**
glibc is backward compatible and not forward compatible, so the version a binary links against is
the oldest it will ever run on: built on a current Ubuntu, `echo-notes` asks for `GLIBC_2.43` and
Debian refuses to start it.

The oldest is not the oldest runner, though. `fastembed` links ONNX Runtime as a prebuilt static
library and that library needs glibc 2.38 — `__isoc23_strtol` and libstdc++'s `_M_replace_cold` do
not exist before it — so on `ubuntu-22.04` the link fails and there is no binary at all. `cargo
check` will not catch that; it never links. `cargo test` and the release build both do.

So both workflows use `ubuntu-24.04` and the floor is its glibc 2.39, which is a fact about the
embedding runtime rather than a choice: **Debian 12 is under it and cannot run these builds; Debian
13 is over it.** Lowering it means supplying an ONNX Runtime built against an older glibc — the
official releases are manylinux — through `ORT_LIB_PATH` instead of the one ort downloads. Until
then `release.yml`'s `glibc floor` step reads the built binary's symbols back and fails if they ask
for more than 2.39, so the number moving is always a diff.

`icons/icon.icns` is what the macOS bundler wants and the only bundler that wants it. It is
generated by `scripts/icons.mjs` with everything else in `icons/` and listed in `tauri.conf.json`'s
`bundle.icon`; a missing one fails the macOS build and nothing else.
