...
For the desktop build, `bun run dev:desktop` opens the Tauri window and `bun run build:desktop` packages it. It also wants a Rust toolchain, and on Linux `webkit2gtk-4.1`, `gtk+-3.0` and `libsoup-3.0`. Tagged builds land under [Releases](https://github.com/luannzin/echo/releases/latest), built by CI rather than by hand — a Linux binary runs only on the glibc it was built against or newer, so one built on your own machine usually starts nowhere but your own machine. The released Linux builds are made on Ubuntu 24.04 and need glibc 2.39 or newer (Debian 13, Ubuntu 24.04, Fedora 39); the floor is ONNX Runtime's, which will not link against anything older.
